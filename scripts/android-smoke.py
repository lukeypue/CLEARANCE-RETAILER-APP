...
def main():
    ...
    print(output)
    if not re.search(r"OK \(11 tests\)", output):
        raise AssertionError("Android data/integration tests did not all pass")
    launch()
    locate("Browse the trial finds.")
    locate("Trial ZIP")
    screenshot("00-trial-discover")
    tap("All trial stores + online  ›", scroll=True)
    tap("Walmart · Harrisville · 2.9 miles")
    tap("All listing categories  ›", scroll=True)
    tap("Clothing")
    locate("STORE PICKUP LISTING", scroll=True)
    screenshot("00-local-pickup-listings")
    tap("Discover tab")
    fill("Trial ZIP", "84043")
    tap("Use this ZIP")
    locate("Trial listings currently cover 84414 only. Use the Stores tab to find mapped locations for 84043.", scroll=True)
    tap("Discover tab")
    fill("Trial ZIP", "84414")
    tap("Use this ZIP")
    adb("shell", "svc", "wifi", "disable")
    adb("shell", "svc", "data", "disable")
    tap("Check for feed updates", scroll=True)
    locate("Could not update the feed. Showing previous dated results; check your connection and try again.", scroll=True, timeout=60)
    locate("STORE PICKUP LISTING", scroll=True)
    screenshot("00-offline-trial")
    adb("shell", "svc", "wifi", "enable")
    adb("shell", "svc", "data", "enable")
    tap("Stores tab")
